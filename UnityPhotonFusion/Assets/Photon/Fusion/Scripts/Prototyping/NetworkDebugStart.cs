...
public class NetworkDebugStart : Fusion.Behaviour {
    ...
    public int userRank=1; //Kullanıcı ID'si rank değerini sembolize eder
  ...
  protected virtual void Start() {
    ...
    if (StartMode == StartModes.Automatic) {
      if (TryGetSceneRef(out var sceneRef)) {
        StartCoroutine(StartWithClients(AutoStartAs, sceneRef, isMultiPeer ? AutoClients : (AutoStartAs == GameMode.Client || AutoStartAs == GameMode.AutoHostOrClient || AutoStartAs == GameMode.Shared  ? 1 : 0)));
      }
    } else {
  ...
  protected virtual Task InitializeNetworkRunner(NetworkRunner runner, GameMode gameMode, NetAddress address, SceneRef scene, Action<NetworkRunner> initialized) {
    ...
    if (sceneManager == null) {
      ...
      sceneManager = runner.gameObject.AddComponent<NetworkSceneManagerDefault>();
        ...
        userRank = 2; //firebase'den gelen veri.

        var customProps = new Dictionary<string, SessionProperty>();
         
        customProps["rank"] = userRank; //Normalde firebase'den gelen rank değeri girilecek fakat ben şimdilik buraya userId değeri girdim ve her build üzerinde girilen userId değerini değiştirdim.

        customProps = new Dictionary<string, SessionProperty>() { //Burada sözlüğe daha önce girilmiş olan rank değeri çekilir
    { "rank", (int)userRank }
  };
